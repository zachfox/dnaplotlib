...
def control (ax, type, num, from_part, to_part, scale, linewidth, arc_height_index, opts):
    """ Standard control regulation renderer.
    """
    regulation(ax, type, num, from_part, to_part, scale, linewidth, arc_height_index, opts)
...
def regulation (ax, type, num, from_part, to_part, scale, linewidth, arc_height_index, opts):
    ...
    line_ind2    = Line2D([end+arrowhead_length,end],[arcHeightEnd+indHeight,arcHeightEnd],
                linewidth=linewidth, color=color, zorder=12, linestyle='-')
    line_control = Line2D([start,end-arrowhead_length],[top,base+arrowhead_length*2],
                linewidth=linewidth, color=color, zorder=12, linestyle=linestyle)
    square_angle = -(np.arctan((base+arrowhead_length*2-top)/(start-(end-arrowhead_length)))*180.0/np.pi+45)

    square_control = Rectangle((end-arrowhead_length,base+arrowhead_length*2),width=arrowhead_length,
                    height=arrowhead_length,angle=square_angle, facecolor='dodgerblue',edgecolor='dodgerblue')
    if(type == 'Repression'):
    ...
    if(type == 'Control'):
        ax.add_line(line_control)
        ax.add_patch(square_control)
            ...
            for reg in regs:
                ...
                if 'type' in keys and 'from_part' in keys and 'to_part' in keys:
                    ...
                    if reg['type'] in list(reg_renderers.keys()):
                        ...
                        reg['arclength'] = np.max([.1,math.fabs(arcstart-arcend)])
